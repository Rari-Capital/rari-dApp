...
describe("FuseSafeLiquidator", function () {
  ...
  before(async function () {
    ...
    comptroller = new fuse.web3.eth.Contract(
      JSON.parse(
    ...
    simplePriceOracle = new fuse.web3.eth.Contract(
      JSON.parse(
  ...
  async function setupUnhealthyEthBorrowWithTokenCollateral() {
    // Supply DAI
    var token = new fuse.web3.eth.Contract(
      erc20Abi,
      "0x6b175474e89094c44da98b954eedeac495271d0f"
    );
    var cToken = new fuse.web3.eth.Contract(cErc20Abi, assetAddresses["fDAI"]);
    await token.methods
      .approve(cToken.options.address, Fuse.Web3.utils.toBN(1e18))
      .send({ from: accounts[0], gasPrice: "0" });
    await cToken.methods
      .mint(Fuse.Web3.utils.toBN(1e18))
      .send({ from: accounts[0], gasPrice: "0" });
    ...
    await cToken.methods
      ...
      .send({
        from: accounts[1],
        gasPrice: "0",
        value: Fuse.Web3.utils.toBN(1e16),
      });

    // Borrow ETH using DAI as collateral
    await comptroller.methods
      .enterMarkets([assetAddresses["fDAI"]])
      .send({ from: accounts[0], gasPrice: "0" });
    await cToken.methods
      .borrow(Fuse.Web3.utils.toBN(1e15))
      .send({ from: accounts[0], gasPrice: "0" });
  ...
  async function setupUnhealthyTokenBorrowWithEthCollateral() {
    ...
    await cToken.methods
      ...
      .send({
        from: accounts[0],
        gasPrice: "0",
        value: Fuse.Web3.utils.toBN(1e15),
      });
    ...
    var cToken = new fuse.web3.eth.Contract(cErc20Abi, assetAddresses["fDAI"]);
    await token.methods
      .approve(cToken.options.address, Fuse.Web3.utils.toBN(5e17))
      .send({ from: accounts[1], gasPrice: "0" });
    await cToken.methods
      .mint(Fuse.Web3.utils.toBN(5e17))
      .send({ from: accounts[1], gasPrice: "0" });

    // Borrow DAI using ETH as collateral
    await comptroller.methods
      .enterMarkets([assetAddresses["fETH"]])
      .send({ from: accounts[0], gasPrice: "0" });
    await cToken.methods
      .borrow(Fuse.Web3.utils.toBN(1e17))
      .send({ from: accounts[0], gasPrice: "0" });
  ...
  async function setupUnhealthyTokenBorrowWithTokenCollateral() {
    ...
    await token.methods
      .approve(cToken.options.address, Fuse.Web3.utils.toBN(1e6))
      .send({ from: accounts[1], gasPrice: "0" });
    await cToken.methods
      .mint(Fuse.Web3.utils.toBN(1e6))
      .send({ from: accounts[1], gasPrice: "0" });
    ...
    await comptroller.methods
      ...
      .send({ from: accounts[0], gasPrice: "0" });
    await cToken.methods
      .borrow(Fuse.Web3.utils.toBN(1e5))
      .send({ from: accounts[0], gasPrice: "0" });
  ...
  async function setupAndLiquidateUnhealthyEthBorrowWithTokenCollateral(
    exchangeTo,
    flashLoan
  ) {
    await setupUnhealthyEthBorrowWithTokenCollateral();
    if (exchangeTo === undefined)
      exchangeTo = "0x6b175474e89094c44da98b954eedeac495271d0f";
    const liquidatorBalanceBeforeLiquidation =
      exchangeTo === "0x0000000000000000000000000000000000000000"
        ? Fuse.Web3.utils.toBN(await fuse.web3.eth.getBalance(accounts[0]))
        : Fuse.Web3.utils.toBN(
            await new fuse.web3.eth.Contract(erc20Abi, exchangeTo).methods
              .balanceOf(accounts[0])
              .call()
          );
    await (flashLoan
      ? fuse.contracts.FuseSafeLiquidator.methods
          .safeLiquidateToEthWithFlashLoan(
            accounts[0],
            Fuse.Web3.utils.toBN(1e14),
            assetAddresses["fETH"],
            assetAddresses["fDAI"],
            0,
            exchangeTo
      ...
      : fuse.contracts.FuseSafeLiquidator.methods
          ...
          .send({
            from: accounts[0],
            gasPrice: "0",
            value: Fuse.Web3.utils.toBN(1e14),
          }));
    ...
    assert(
      liquidatorBalanceAfterLiquidation.gt(liquidatorBalanceBeforeLiquidation)
    );
  }

  async function setupAndLiquidateUnhealthyTokenBorrowWithEthCollateral(
    exchangeTo,
    flashLoan
  ) {
    ...
    if (flashLoan)
      await fuse.contracts.FuseSafeLiquidator.methods
        .safeLiquidateToTokensWithFlashLoan(
          accounts[0],
          Fuse.Web3.utils.toBN(1e16),
          assetAddresses["fDAI"],
          assetAddresses["fETH"],
          0,
          exchangeTo
      ...
      await token.methods
        .approve(
          fuse.contracts.FuseSafeLiquidator.options.address,
          Fuse.Web3.utils.toBN(1e16)
        )
        .send({ from: accounts[0], gasPrice: "0" });
      await fuse.contracts.FuseSafeLiquidator.methods
  ...
  async function setupAndLiquidateUnhealthyTokenBorrowWithTokenCollateral(
    exchangeTo,
    flashLoan
  ) {
    ...
    if (flashLoan)
      await fuse.contracts.FuseSafeLiquidator.methods
        .safeLiquidateToTokensWithFlashLoan(
          accounts[0],
          Fuse.Web3.utils.toBN(1e4),
          assetAddresses["fUSDC"],
          assetAddresses["fDAI"],
          0,
          exchangeTo
        )
  ...
  describe("#safeLiquidate()", function () {
    ...
    it(
      "should liquidate an ETH borrow for token collateral and exchange to ETH",
      dryRun(async () => {
        await setupAndLiquidateUnhealthyEthBorrowWithTokenCollateral(
          "0x0000000000000000000000000000000000000000"
        );
    ...
    it(
      "should liquidate an ETH borrow for token collateral and exchange to another token",
      dryRun(async () => {
        await setupAndLiquidateUnhealthyEthBorrowWithTokenCollateral(
    ...
    it(
      "should liquidate a token borrow for ETH collateral and exchange to another token",
      dryRun(async () => {
        await setupAndLiquidateUnhealthyTokenBorrowWithEthCollateral(
          "0xD291E7a03283640FDc51b121aC401383A46cC623"
        );
    ...
    it(
      "should liquidate a token borrow for token collateral and exchange to ETH",
      dryRun(async () => {
        await setupAndLiquidateUnhealthyTokenBorrowWithTokenCollateral(
          "0x0000000000000000000000000000000000000000"
        );
    ...
    it(
      "should liquidate a token borrow for token collateral and exchange to another token",
      dryRun(async () => {
        await setupAndLiquidateUnhealthyTokenBorrowWithTokenCollateral(
          "0xD291E7a03283640FDc51b121aC401383A46cC623"
        );
  ...
  describe("#safeLiquidateToEthWithFlashLoan()", function () {
    ...
    it(
      "should liquidate an ETH borrow (using a flash swap) for token collateral and exchange to ETH",
      dryRun(async () => {
        await setupAndLiquidateUnhealthyEthBorrowWithTokenCollateral(
          "0x0000000000000000000000000000000000000000",
          true
    ...
    it(
      "should liquidate an ETH borrow (using a flash swap) for token collateral and exchange to another token",
      dryRun(async () => {
        await setupAndLiquidateUnhealthyEthBorrowWithTokenCollateral(
          "0xD291E7a03283640FDc51b121aC401383A46cC623",
          true
  ...
  describe("#safeLiquidateToTokensWithFlashLoan()", function () {
    ...
    it(
      "should liquidate a token borrow (using a flash swap) for ETH collateral and exchange to another token",
      dryRun(async () => {
        await setupAndLiquidateUnhealthyTokenBorrowWithEthCollateral(
          "0xD291E7a03283640FDc51b121aC401383A46cC623",
          true
    ...
    it(
      "should liquidate a token borrow (using a flash swap) for token collateral and exchange to ETH",
      dryRun(async () => {
        await setupAndLiquidateUnhealthyTokenBorrowWithTokenCollateral(
          "0x0000000000000000000000000000000000000000",
          true
    ...
    it(
      "should liquidate a token borrow (using a flash swap) for token collateral and exchange to another token",
      dryRun(async () => {
        await setupAndLiquidateUnhealthyTokenBorrowWithTokenCollateral(
          "0xD291E7a03283640FDc51b121aC401383A46cC623",
          true
