...
export default class AlphaSubpool {
  ...
  async getIBEthApyBN() {
    ...
    return await this.cache.getOrUpdate("alphaIBEthApy", async function () {
      try {
        var glbDebtVal = await self.externalContracts.Bank.methods.glbDebtVal().call();
        var balance = await self.web3.eth.getBalance(self.externalContracts.Bank.options.address);
        var interestRatePerSecondBN = await self.externalContracts.ConfigurableInterestBankConfig.methods.getInterestRate(glbDebtVal, balance).call();
        return Web3.utils.toBN(Math.trunc((((1 + (interestRatePerSecondBN / 1e18)) ** (365 * 24 * 60 * 60)) - 1) * 1e18));
      } catch (err) {
