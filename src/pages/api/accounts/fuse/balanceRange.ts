...
import {
  getAllUserBalances,
  getUserBalancesByBlockRange,
  getUserBalancesByDateRange,
  setupDB,
} from "utils/api/db";
  ...
  if (req.method === "GET") {
    try {
      ...
      if (startBlock && endBlock) {
        data = await getUserBalancesByBlockRange(startBlock, endBlock);
      } else if (startBlock) {
        data = await getUserBalancesByBlockRange(startBlock, latestBlockNumber);
      } else if (startDate && endDate) {
        console.log({ startDate, endDate });
        data = await getUserBalancesByDateRange(startDate, endDate);
      } else if (startDate) {
        data = await getUserBalancesByDateRange(startDate, currentDate);
      } else {
        data = await getAllUserBalances();
      }
